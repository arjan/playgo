...
$.extend(PlayGo.prototype, {
    ...
    addPoints: function() {
        var marker, markerDragEnd, points = this.points, point;

        $(points).each($.proxy(function(i) {
            point = points[i];
            marker = this.addMarker(point.lat, point.long, point.ts);
    ...
    drawPolyLine: function() {
        ...
        for (var i = 0; i < points.length; i++) {
            latLngs.push(new google.maps.LatLng(points[i].lat, points[i].long));
        }
